...
import numpy as np
import pytest
from player.mixer import Mixer, DC_OFFSET


class TestMixer:
    def test_silence_is_dc_offset(self):
        mixer = Mixer()
        zeros = [np.zeros(100, dtype=np.int16) for _ in range(4)]
        out = mixer.mix(zeros)
    ...
    def test_single_channel(self):
        ...
        out = mixer.mix([ch, zeros, zeros, zeros])
        assert np.all(out == DC_OFFSET + 64)

    def test_no_overflow(self):
        mixer = Mixer()
        maxvol = np.full(100, 127, dtype=np.int16)
        out = mixer.mix([maxvol, maxvol, maxvol, maxvol])
        assert out.max() <= 1023
        assert out.min() >= 0
    ...
    def test_no_underflow(self):
        ...
        out = mixer.mix([minvol, minvol, minvol, minvol])
        assert out.min() >= 0
        assert out.max() <= 1023
